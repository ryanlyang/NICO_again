...
#SBATCH --mem=64G
#SBATCH --output=logsNICO/nico_vanilla_optuna_%j.out
#SBATCH --error=logsNICO/nico_vanilla_optuna_%j.err
...
STUDY_NAME=${STUDY_NAME:-nico_vanilla_${TARGET_TAG}}
OPTUNA_STORAGE=${OPTUNA_STORAGE:-sqlite:///$OUTPUT_DIR/vanilla_optuna_${TARGET_TAG}.db}
...
if [[ ! -f "$REPO_ROOT/run_nico_vanilla_optuna.py" ]]; then
  echo "Missing run_nico_vanilla_optuna.py in $REPO_ROOT" >&2
  exit 1
fi
...
srun --unbuffered python -u run_nico_vanilla_optuna.py \
  --txtdir "$TXTLIST_DIR" \
  --dataset "$DATASET" \
