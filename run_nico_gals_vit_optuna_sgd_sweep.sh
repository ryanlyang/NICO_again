#!/bin/bash -l
# NICO GALS (ViT attention) Optuna SGD sweep
...
STUDY_NAME=${STUDY_NAME:-nico_gals_vit_sgd_${TARGET_TAG}}
...
if [[ ! -f "$REPO_ROOT/run_nico_gals_vit_optuna_sgd.py" ]]; then
  echo "Missing run_nico_gals_vit_optuna_sgd.py in $REPO_ROOT" >&2
  exit 1
fi
