...
JOB_SCRIPT="$SCRIPT_DIR/run_nico_guided_optuna_sgd_valacc_sweep.sh"
...
for TARGET in "autumn rock" "dim grass" "outdoor water"; do
  ...
  STUDY_NAME="nico_guided_sgd_valacc_locked_vanilla_${TARGET_TAG}" \
  OPTUNA_STORAGE="${OPTUNA_STORAGE:-sqlite:///$OUTPUT_DIR/optuna_guided_sgd_valacc_locked_vanilla_v1_${TARGET_TAG}.db}" \
  sbatch "$JOB_SCRIPT"
done
