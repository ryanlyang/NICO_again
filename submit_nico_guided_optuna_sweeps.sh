...
for TARGET in "autumn rock" "dim grass" "outdoor water"; do
  ...
  sbatch --export=ALL,REPO_ROOT="$REPO_ROOT",TXTLIST_DIR="$TXTLIST_DIR",MASK_ROOT="$MASK_ROOT",OUTPUT_DIR="$OUTPUT_DIR",TARGET_DOMAINS="$TARGET",TARGET_TAG="$TARGET_TAG",TIMEOUT_SECONDS="$TIMEOUT_SECONDS",N_TRIALS="$N_TRIALS",OPTUNA_STORAGE="${OPTUNA_STORAGE:-sqlite:///$OUTPUT_DIR/optuna_${TARGET_TAG}.db}" \
    "$JOB_SCRIPT"
done
