...
# =============================================================================
# BASE SETTINGS (match the guided Table-9-ish protocol by default)
# =============================================================================
TOTAL_STEPS = 10000
...
def main():
    ...
    # Search space (keep small + sensible; extend as needed)
    parser.add_argument("--lr_low", type=float, default=1e-5)
    parser.add_argument("--lr_high", type=float, default=3e-4)
    parser.add_argument("--wd_low", type=float, default=0.0)
    parser.add_argument("--wd_high", type=float, default=1e-3)
    parser.add_argument("--bs_choices", type=str, default="16,32,64", help="Comma-separated batch sizes to search.")
    ...
    bs_choices = [int(x) for x in args.bs_choices.split(",") if x.strip()]
    ...
    def objective(trial):
        ...
        if args.wd_low == 0.0 and args.wd_high == 0.0:
            weight_decay = 0.0
        else:
            # Allow exact 0.0 while still mostly sampling log-space.
            if args.wd_low == 0.0:
                use_zero = trial.suggest_categorical("wd_is_zero", [True, False])
                if use_zero:
                    weight_decay = 0.0
                else:
                    weight_decay = suggest_loguniform(trial, "weight_decay", 1e-8, args.wd_high)
            else:
        ...
        print(
            f"[TRIAL {trial.number}] start params={{'lr': {lr}, 'weight_decay': {weight_decay}, "
            f"'batch_size': {batch_size}, 'resnet_dropout': {resnet_dropout}, 'num_epochs': {num_epochs}}}",
            flush=True,
        )
        ...
        trial_args = argparse.Namespace(lr=lr, weight_decay=weight_decay, num_epochs=num_epochs)
    ...
    if int(args.rerun_best) == 1:
        ...
        best_wd = resolve_weight_decay(study.best_params)
        best_bs = int(study.best_params.get("batch_size", bs_choices[0]))
        ...
        rerun_header = ["seed", "lr", "weight_decay", "batch_size", "resnet_dropout", "num_epochs", "best_val_acc", "test_results_json", "minutes"]
        ...
        print(
            f"Best params resolved: lr={best_lr} weight_decay={best_wd} batch_size={best_bs} resnet_dropout={best_dropout}",
            flush=True,
        )
        ...
        for seed in seeds:
            ...
            steps_per_epoch = max(1, math.ceil(dataset_sizes_s["train"] / best_bs))
            num_epochs = max(1, math.ceil(TOTAL_STEPS / steps_per_epoch))

            dataloaders_s = {
                "train": DataLoader(
                    train_dataset_s, batch_size=best_bs, shuffle=True,
                    num_workers=args.num_workers, worker_init_fn=seed_worker, generator=g
                ),
                "val": DataLoader(
                    val_dataset_s, batch_size=best_bs, shuffle=False,
                    num_workers=args.num_workers, worker_init_fn=seed_worker, generator=g
                ),
            }
            ...
            test_loaders = [
                DataLoader(
                    ds, batch_size=best_bs, shuffle=False,
                    num_workers=args.num_workers, worker_init_fn=seed_worker, generator=g
                )
                for ds in test_datasets
            ]
            ...
            run_args = argparse.Namespace(lr=best_lr, weight_decay=best_wd, num_epochs=num_epochs)
