...
OUTPUT_DIR=${OUTPUT_DIR:-/home/ryreu/guided_cnn/NICO_runs/seeded_runs_optuna}
...
if [[ ! -f "$REPO_ROOT/run_nico_guided_optuna_multi_seed.py" ]]; then
  echo "Missing run_nico_guided_optuna_multi_seed.py in $REPO_ROOT" >&2
  exit 1
fi
...
srun --unbuffered python -u run_nico_guided_optuna_multi_seed.py "${ARGS[@]}"
