#!/bin/bash -l
# Guided post-attention SWAD Optuna sweep with SGD
...
#SBATCH --mem=32G
#SBATCH --output=logsNICO/nico_guided_swad_post_sgd_optuna_%j.out
#SBATCH --error=logsNICO/nico_guided_swad_post_sgd_optuna_%j.err
...
STUDY_NAME=${STUDY_NAME:-nico_guided_swad_post_sgd_${TARGET_TAG}}
OPTUNA_STORAGE=${OPTUNA_STORAGE:-sqlite:///$OUTPUT_DIR/optuna_guided_swad_post_sgd_v1_${TARGET_TAG}.db}
...
if [[ ! -f "$REPO_ROOT/run_nico_guided_swad_post_sgd_optuna.py" ]]; then
  echo "Missing run_nico_guided_swad_post_sgd_optuna.py in $REPO_ROOT" >&2
  exit 1
fi
...
srun --unbuffered python -u run_nico_guided_swad_post_sgd_optuna.py \
  --txtdir "$TXTLIST_DIR" \
  --dataset "$DATASET" \
