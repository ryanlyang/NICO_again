#!/bin/bash -l
# NICO vanilla ERM Optuna sweep (single shared LR for base+classifier)
...
#SBATCH --mem=32G
#SBATCH --output=logsNICO/nico_vanilla_singlelr_sgd_optuna_%j.out
#SBATCH --error=logsNICO/nico_vanilla_singlelr_sgd_optuna_%j.err
...
STUDY_NAME=${STUDY_NAME:-nico_vanilla_singlelr_sgd_${TARGET_TAG}}
...
srun --unbuffered python -u run_vanilla_optuna_sgd.py \
  ...
  --run_name_prefix "vanilla_singlelr" \
