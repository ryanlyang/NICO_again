...
JOB_SCRIPT="$SCRIPT_DIR/run_nico_vanilla_optuna_sgd_sweep.sh"
...
for TARGET in "autumn" "rock" "dim" "grass" "outdoor" "water"; do
  ...
  sbatch --export=ALL,REPO_ROOT="$REPO_ROOT",TXTLIST_DIR="$TXTLIST_DIR",IMAGE_ROOT="$IMAGE_ROOT",OUTPUT_DIR="$OUTPUT_DIR",TARGET_DOMAINS="$TARGET",TARGET_TAG="$TARGET_TAG",TIMEOUT_SECONDS="$TIMEOUT_SECONDS",N_TRIALS="$N_TRIALS",FRESH_START=1,STUDY_NAME="nico_vanilla_sgd_${TARGET_TAG}",OPTUNA_STORAGE="${OPTUNA_STORAGE:-sqlite:///$OUTPUT_DIR/vanilla_sgd_optuna_v1_${TARGET_TAG}.db}" \
    "$JOB_SCRIPT"
done
