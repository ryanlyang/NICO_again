...
JOB_SCRIPT="$SCRIPT_DIR/run_nico_gals_vit_optuna_sgd_optim_sweep.sh"
...
for TARGET in "autumn rock" "dim grass" "outdoor water"; do
  ...
  sbatch --export=ALL,REPO_ROOT="$REPO_ROOT",TXTLIST_DIR="$TXTLIST_DIR",IMAGE_ROOT="$IMAGE_ROOT",ATTENTION_ROOT="$ATTENTION_ROOT",OUTPUT_DIR="$OUTPUT_DIR",TARGET_DOMAINS="$TARGET",TARGET_TAG="$TARGET_TAG",TIMEOUT_SECONDS="$TIMEOUT_SECONDS",N_TRIALS="$N_TRIALS",RERUN_BEST="$RERUN_BEST",RERUN_NUM_SEEDS="$RERUN_NUM_SEEDS",RERUN_SEED_START="$RERUN_SEED_START",FRESH_START="$FRESH_START",STUDY_NAME="nico_gals_vit_sgd_optim_${TARGET_TAG}",OPTUNA_STORAGE="${OPTUNA_STORAGE:-sqlite:///$OUTPUT_DIR/optuna_nico_gals_vit_sgd_optim_v1_${TARGET_TAG}.db}" \
    "$JOB_SCRIPT"
done
