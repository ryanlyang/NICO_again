...
def compute_attn_losses(saliency: torch.Tensor, masks: torch.Tensor):
    """
    KL terms between saliency and masks.
      saliency: (B,H,W) in [0,1]
      masks:    (B,H,W) in [0,1]
    """
    B = saliency.size(0)
    sal_flat = saliency.view(B, -1)
    mask_flat = masks.view(B, -1)

    sal_prob = sal_flat / (sal_flat.sum(dim=1, keepdim=True) + 1e-8)
    log_sal = torch.log(sal_prob + 1e-8)
    mask_prob = mask_flat / (mask_flat.sum(dim=1, keepdim=True) + 1e-8)
    log_mask = torch.log(mask_prob + 1e-8)

    kl_div = nn.KLDivLoss(reduction="batchmean")
    forward_kl = kl_div(log_sal, mask_prob)
    reverse_kl = kl_div(log_mask, sal_prob)
    return forward_kl, reverse_kl


def input_grad_saliency(model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor):
    """
...
def main():
    ...
    run_name = f"gals_vit_optim_target_{'-'.join(args.target)}"
    ...
    if int(args.rerun_best) == 1:
        ...
        for seed in seeds:
            ...
            model.load_state_dict(best_wts_optim)
            test_results_optim = evaluate_test(model, test_loaders_s, args.target)
            model.load_state_dict(best_wts_val)
            test_results_valacc = evaluate_test(model, test_loaders_s, args.target)
