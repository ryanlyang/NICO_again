...
with optional pre-attention checkpoint caching.
...
def _cache_ckpt_path(cache_dir, epoch):
    return os.path.join(cache_dir, f"pre_att_epoch_{int(epoch):02d}.pt")
...
def _build_pre_attention_cache(
    ...
    """
    Build CE-only checkpoints for epochs [0..max_pre_epoch], so trials can warm-start from
    epoch (attention_epoch - 1) and only run the post-attention phase.
    """
    os.makedirs(cache_dir, exist_ok=True)
    meta_path = os.path.join(cache_dir, "cache_meta.json")

    # Reuse cache if complete and matching settings.
    expected_paths = [_cache_ckpt_path(cache_dir, e) for e in range(max_pre_epoch + 1)]
    if os.path.exists(meta_path) and all(os.path.exists(p) for p in expected_paths) and int(args.pre_cache_force_rebuild) != 1:
        with open(meta_path, "r") as f:
            meta = json.load(f)
        if (
            float(meta.get("base_lr", -1.0)) == float(base_lr)
            and float(meta.get("classifier_lr", -1.0)) == float(classifier_lr)
            and int(meta.get("max_pre_epoch", -1)) == int(max_pre_epoch)
            and float(meta.get("resnet_dropout", -1.0)) == float(args.resnet_dropout)
            and int(meta.get("seed", -1)) == int(args.seed)
        ):
            print(f"[PRE-CACHE] Reusing existing cache at {cache_dir}", flush=True)
            return

    print(
        f"[PRE-CACHE] Building CE-only cache at {cache_dir} "
        f"(epochs 0..{max_pre_epoch}, base_lr={base_lr}, classifier_lr={classifier_lr})",
        flush=True,
    )
    ...
    try:
        for epoch in range(max_pre_epoch + 1):
            for phase in ["train", "val"]:
                ...
                if is_train:
                    ...
                    val_acc_by_epoch[str(epoch)] = float(epoch_acc)
            ...
            ckpt = {
                ...
                "best_val_acc_up_to_now": float(best_val_acc),
                "best_val_epoch_up_to_now": int(best_val_epoch),
                "val_acc_this_epoch": float(val_acc_by_epoch[str(epoch)]),
            ...
            torch.save(ckpt, _cache_ckpt_path(cache_dir, epoch))
            print(
                f"[PRE-CACHE][Epoch {epoch}] val_acc={val_acc_by_epoch[str(epoch)]:.6f} "
                f"best_val={best_val_acc:.6f}@{best_val_epoch}",
                flush=True,
            )
    finally:
        del model
    ...
    meta = {
        ...
        "max_pre_epoch": int(max_pre_epoch),
        "resnet_dropout": float(args.resnet_dropout),
        "seed": int(args.seed),
    ...
    print(f"[PRE-CACHE] Done. Meta: {meta_path}", flush=True)


def train_one_trial_from_cache(
    model,
    train_dataset,
    val_dataset,
    dataset_sizes,
    ...
    """
    Warm-start trial from cached CE-only checkpoint at epoch (attention_epoch-1),
    then run only epochs [attention_epoch .. num_epochs-1] with guided objective.
    """
    att_epoch = int(args.attention_epoch)
    if att_epoch < 1:
        raise ValueError(f"attention_epoch must be >=1 for cached training, got {att_epoch}")

    pre_ckpt_path = _cache_ckpt_path(cache_dir, att_epoch - 1)
    ...
    if "torch_rng_state" in pre_ckpt:
        torch.set_rng_state(pre_ckpt["torch_rng_state"])
    if torch.cuda.is_available() and "cuda_rng_state_all" in pre_ckpt:
        torch.cuda.set_rng_state_all(pre_ckpt["cuda_rng_state_all"])
    if "numpy_rng_state" in pre_ckpt:
        np.random.set_state(pre_ckpt["numpy_rng_state"])
    if "python_rng_state" in pre_ckpt:
        random.setstate(pre_ckpt["python_rng_state"])

    g = torch.Generator()
    if "generator_state" in pre_ckpt:
        g.set_state(pre_ckpt["generator_state"])
    else:
        g.manual_seed(args.seed)
    ...
    best_val_acc = pre_best_val_acc
    best_log_optim_value = float("-inf")
    best_wts_optim = copy.deepcopy(pre_ckpt["model_state_dict"])
    ...
    for epoch in range(att_epoch, args.num_epochs):
...
def main():
    ...
    parser.add_argument('--pre_attention_cache', type=int, default=1,
                        help='Build/reuse CE-only pre-attention checkpoints and warm-start each trial (1/0).')
    parser.add_argument('--pre_cache_force_rebuild', type=int, default=0,
                        help='Force rebuilding the pre-attention cache (1/0).')
    parser.add_argument('--pre_attention_cache_dir', type=str, default='',
                        help='Optional explicit cache directory; default is <output_dir>/pre_attention_cache.')
    ...
    use_pre_cache = False
    pre_cache_dir = ""
    # Cache mode is only valid when pre-attention dynamics are fixed across trials.
    # That requires locked base/classifier LR ranges.
    if int(args.pre_attention_cache) == 1:
        locked_base = abs(float(args.base_lr_low) - float(args.base_lr_high)) < 1e-18
        locked_cls = abs(float(args.classifier_lr_low) - float(args.classifier_lr_high)) < 1e-18
        if locked_base and locked_cls:
            use_pre_cache = True
            fixed_base_lr = float(args.base_lr_low)
            fixed_classifier_lr = float(args.classifier_lr_low)
            pre_cache_dir = args.pre_attention_cache_dir.strip() or os.path.join(output_dir, "pre_attention_cache")
            max_pre_epoch = max(0, int(args.att_epoch_max) - 1)
            if max_pre_epoch >= 0:
                _build_pre_attention_cache(
                    args=args,
                    train_dataset=train_dataset,
                    val_dataset=val_dataset,
                    max_pre_epoch=max_pre_epoch,
                    base_lr=fixed_base_lr,
                    classifier_lr=fixed_classifier_lr,
                    cache_dir=pre_cache_dir,
                )
                print(
                    f"[PRE-CACHE] Enabled for trials (cache_dir={pre_cache_dir}, max_pre_epoch={max_pre_epoch})",
                    flush=True,
                )
        else:
            print(
                "[PRE-CACHE] Disabled: base/classifier LR are not locked; "
                "falling back to full per-trial training.",
                flush=True,
            )
    ...
    def objective(trial):
        ...
        if use_pre_cache:
            # Trials warm-start from cached pre-attention checkpoints; keep seed fixed.
            seed_everything(args.seed)
            dataloaders = None
        else:
        ...
        try:
            if use_pre_cache:
                best_log_optim_value, best_val_acc, best_wts_optim, best_wts_val = train_one_trial_from_cache(
                    model=model,
                    train_dataset=train_dataset,
                    val_dataset=val_dataset,
