...
#SBATCH --mem=96G
#SBATCH --output=logsNICO/nico_guided_metric_debug_autumn_rock_%j.out
#SBATCH --error=logsNICO/nico_guided_metric_debug_autumn_rock_%j.err
...
OUTPUT_DIR=${OUTPUT_DIR:-/home/ryreu/guided_cnn/NICO_runs/output/debug_metric_autumn_rock}
...
if [[ ! -f "$REPO_ROOT/run_nico_guided_metric_debug_autumn_rock.py" ]]; then
  echo "Missing run_nico_guided_metric_debug_autumn_rock.py in $REPO_ROOT" >&2
  exit 2
fi
...
echo "IG steps: $IG_STEPS"
which python

srun --unbuffered python -u run_nico_guided_metric_debug_autumn_rock.py \
  --txtdir "$TXTLIST_DIR" \
  --dataset "$DATASET" \
