...
OUTPUT_DIR=${OUTPUT_DIR:-/home/ryreu/guided_cnn/NICO_runs/seeded_runs_optim_lambda}
...
if [[ ! -f "$REPO_ROOT/run_nico_guided_optim_lambda_multi_seed.py" ]]; then
  echo "Missing run_nico_guided_optim_lambda_multi_seed.py in $REPO_ROOT" >&2
  exit 1
fi
...
srun --unbuffered python -u run_nico_guided_optim_lambda_multi_seed.py "${ARGS[@]}"
