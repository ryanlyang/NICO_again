#!/bin/bash -l
# NICO UpWeight Optuna SGD sweep
...
#SBATCH --mem=128G
#SBATCH --output=logsNICO/nico_upweight_sgd_optuna_%j.out
#SBATCH --error=logsNICO/nico_upweight_sgd_optuna_%j.err
...
STUDY_NAME=${STUDY_NAME:-nico_upweight_sgd_${TARGET_TAG}}
OPTUNA_STORAGE=${OPTUNA_STORAGE:-sqlite:///$OUTPUT_DIR/optuna_nico_upweight_sgd_v1_${TARGET_TAG}.db}
...
if [[ ! -f "$REPO_ROOT/run_nico_upweight_optuna_sgd.py" ]]; then
  echo "Missing run_nico_upweight_optuna_sgd.py in $REPO_ROOT" >&2
  exit 1
fi
